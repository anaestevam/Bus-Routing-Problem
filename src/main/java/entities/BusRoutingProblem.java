...
public class BusRoutingProblem {
    protected static AtomicInteger busCounter = new AtomicInteger(0);
    protected static AtomicInteger nodeCounter = new AtomicInteger(0);
    private int totalBus, totalServicePoints;
    private Node depository;
    private ArrayList<Node> allNodes;
    private ArrayList<Bus> allBus;
    private ArrayList<Route> routes;
    private double[][] distanceMatrix, timeMatrix;

    public BusRoutingProblem(int totalServicePoints, int totalBus)  {
    ...
    private boolean capacityConstraintsAreViolated(Route route1, int index1, Route route2, int index2) {
        ...
        double firstRouteLoad = route1.getBus().getMaxCap() - route1.getBus().getRemainingCap();
        ...
        double secondRouteLoad = route2.getBus().getMaxCap() - route2.getBus().getRemainingCap();
        double secondRouteSecondSegmentLoad = secondRouteLoad - secondRouteFirstSegmentLoad;

        if(firstRouteFirstSegmentLoad + secondRouteSecondSegmentLoad > route1.getBus().getMaxCap()) {
            return true;
        }

        if(secondRouteFirstSegmentLoad + firstRouteSecondSegmentLoad > route2.getBus().getMaxCap()) {
            return true;
        }
        ...
        for(int to = 0; to < totalRoutes; to++) {
            ...
            for(int sourceIndex = 1; sourceIndex < routes.get(from).getRouteSize(); sourceIndex++) {
                ...
                for(int targetIndex = secondIndex; targetIndex < routes.get(to).getRouteSize(); targetIndex++) {
                    ...
                    if(examinedRouteTo.getBus().getRemainingCap() + examinedRouteTo.getRouteNodes().get(targetIndex).getDemand() < examinedRouteFrom.getRouteNodes().get(sourceIndex).getDemand()) {
                        continue;
                    }
                    ...
                    if(criterion < maxTime) {
                        if(moveCostFrom < bestMoveCostFrom && moveCost < 0D) {
                            ...
                            if(b.getDemand() == f.getDemand()) {
                                sm.setFromRemainingCap(examinedRouteFrom.getBus().getRemainingCap()); //ametablhto
                                sm.setToRemainingCap(examinedRouteTo.getBus().getRemainingCap());
                            } else if (b.getDemand() > f.getDemand()) {
                                sm.setFromRemainingCap(examinedRouteFrom.getBus().getRemainingCap() + demandAbsoluteDifference);
                                sm.setToRemainingCap(examinedRouteTo.getBus().getRemainingCap() - demandAbsoluteDifference);
                            } else {
                                sm.setFromRemainingCap(examinedRouteFrom.getBus().getRemainingCap() - demandAbsoluteDifference);
                                sm.setToRemainingCap(examinedRouteTo.getBus().getRemainingCap() + demandAbsoluteDifference);
                            }
    ...
    private Route cloneRoute(Route r) {
        Bus clonedBus = cloneBus(r.getBus());
        Route clonedRoute = new Route(clonedBus);
    ...
    private Bus cloneBus(Bus bus) {
        Bus clonedBus = new Bus();
        clonedBus.setBusID(bus.getBusID());
        clonedBus.setRemainingCap(bus.getRemainingCap());
        return clonedBus;
    }
                ...
                for(int targetIndex = 0; targetIndex < routes.get(to).getRouteSize() - 1; targetIndex++) { //συμβολίζει διαστήματα
                    ...
                    if(criterion < maxTime) {
                        if (moveCostFrom < bestMoveCostFrom) {
                            ...
                            rm.setFromRemainingCap(examinedRouteFrom.getBus().getRemainingCap() + relocatedDemand);
                            rm.setToRemainingCap(examinedRouteTo.getBus().getRemainingCap() - relocatedDemand);
                        }
                    }
                }
            }
    ...
    private void initializeRoutes() {
        routes = new ArrayList<Route>(totalBus);

        for(Bus bus : allBus) {
            //create an initial Route for each bus
            Route route = new Route(bus);
    ...
    private void applyNearestNeighborMethod() {
        for (int i = 0; i < allNodes.size()-1; i++) {
            ...
            do {
                nQuickestRoutes = findNQuickestRoutes(totalBus);
    ...
    private Node findMinUnservicedPoint(Route currentQuickestRoute) {
        Bus bus = currentQuickestRoute.getBus();
        int quickestRouteCurrentNode = currentQuickestRoute.getCurrentNode().getNodeID();
        double min = Double.MAX_VALUE;
        int minIndex = -1;
        for(int j = 1; j < allNodes.size(); j++) {
            if (!allNodes.get(j).isServiced() && allNodes.get(j).getDemand() <= bus.getRemainingCap()) {
